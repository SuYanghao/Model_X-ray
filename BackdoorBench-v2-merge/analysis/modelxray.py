...
parser.add_argument("--num_plot", type=int, default=1,help="How many decision boundary plots to draw")
parser.add_argument('--resolution', default=200, type=float, help='resolution for plot')
...
def produce_plot_alt(args,path, preds, planeloader, images, labels, temp=1.0):
...
def produce_plot_sepleg(path, preds, planeloader, images, labels, temp=0.01,true_labels = None):
...
# Set num_classes
if 'cifar100' in args.result_file:
    args.num_classes=100

elif 'gtsrb' in args.result_file:
    args.num_classes=43   

elif 'cifar10' in args.result_file:
    args.num_classes=10  

elif 'tiny' in args.result_file:
...
print(f"Load model {args.model} from {args.result_file}")
...
for num in range(args.num_plot):
    ...
    #Visualization
    plot_path  = save_path_attack + f'/decision_boundary_{args.result_file}_{num}'

    #plot
    produce_plot_alt(args,plot_path, preds, planeloader, images, labels)
    print(f"The plotted decision boundary has been saved to {plot_path}")
